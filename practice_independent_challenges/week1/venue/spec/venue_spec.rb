...
describe Venue do
  ...
  describe '#over_capacity?' do
    it 'returns a boolean depending if parameter is met' do
      ...
      venue.add_patron('Kelly')
      expect(venue.over_capacity?).to eq(true)
    end
  end

  describe '#kick_out' do
    it 'removes patrons until number meets capacity criteria' do
    venue=Venue.new('Bluebird', 4)
      venue.add_patron('Mike')
      venue.add_patron('Megan')
      venue.add_patron('Bob')
      venue.add_patron('Jim')
      venue.add_patron('Kelly')
      # expect(venue.patrons).to eq(['Mike', 'Megan', 'Bob', 'Jim', 'Kelly'])
      expect(venue.over_capacity?).to eq(true)
      venue.kick_out
      # expect(venue.patrons).to eq(['Mike', 'Megan', 'Bob', 'Jim'])
